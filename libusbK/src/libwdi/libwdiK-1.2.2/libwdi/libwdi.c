...
DLL_DECLARE(WINAPI, LPSTR, PathFindExtensionA, (LPSTR));
...
static int init_dlls(void)
{
	...
	DLL_LOAD(Shlwapi.dll, PathFindExtensionA, FALSE);
							   ...
							   struct wdi_options_create_list* options)
{
	...
	for (i = 0; ; i++)
	{
		...
		if (safe_stricmp(strbuf, usbccgp_name) == 0) {
			/* 
			Listing hubs is a bad idea; we need usbccgp! 
		...
		if ((device_info->desc != NULL) && (options != NULL) && (options->trim_whitespaces)) {
			...
			while ((end != device_info->desc) && isspace((unsigned char)*(end-1))) {
				--end;
			}
			*end = 0;
		}
...
static int extract_binaries(char* path, int driver_type, DWORD package_flags)
{
	FILE *fd;
	char filename[MAX_PATH];
	int i, r;
	int driver_type_res;
	DWORD package_flags_res;
	LPSTR pathExt;
	...
	for (i=0; i<nb_resources; i++) {
		...
		driver_type_res = resource[i].driver_type & (~WDI_PACKAGE_FLAG_MASK);
		package_flags_res = (DWORD)resource[i].driver_type & WDI_PACKAGE_FLAG_MASK;

		// Ignore files not needed for this driver type
		if (driver_type_res!=WDI_ALL_DRIVERS && driver_type < WDI_USER &&  driver_type != driver_type_res) {
			continue;
		}

		//Ignore files not included for this package type
		if ((package_flags_res & package_flags)==0) {
			continue;
		}
		...
		pathExt = PathFindExtensionA(resource[i].subdir);
		while(pathExt && pathExt[0]=='.')
			pathExt = &pathExt[1];
		...
		if (safe_strlen(filename) > (MAX_PATH - 3)) {
								  ...
								  char* inf_name, struct wdi_options_prepare_driver* options)
{
	...
	const char* def_inf_class_guid;
	const char* def_inf_class;
	const char* def_inf_provider;
	...
	int driver_type = WDI_WINUSB, r = WDI_ERROR_OTHER;
	DWORD package_flags = 0;
	long inf_file_size;
	FILE* fd;
	GUID guid;
	...
	if (options != NULL) {
		driver_type   = options->driver_type & (~WDI_PACKAGE_FLAG_MASK);
		package_flags = options->driver_type & WDI_PACKAGE_FLAG_MASK;
	}

	if (package_flags==0)
		package_flags = WDI_PACKAGE_FLAG_CREATE_NORMAL|WDI_PACKAGE_FLAG_FOR_INSTALL;
	...
	if (driver_type == WDI_USER) {
		...
		MUTEX_RETURN extract_binaries(path,driver_type,package_flags);
	}
	...
	r = extract_binaries(path,driver_type,package_flags);
	if (r != WDI_SUCCESS) {
		MUTEX_RETURN r;
	}
	...
	{
	case WDI_WINUSB:
		def_inf_class_guid	= "{88BAE032-5A81-49F0-BC3D-A4FF138216D6}";
		def_inf_class		= "Universal Serial Bus devices";
		def_inf_provider	= "libusb.org";
		break;
	case WDI_LIBUSB0:
		def_inf_class_guid	= "{EB781AAF-9C70-4523-A5DF-642A87ECA567}";
		def_inf_class		= "libusb-win32 devices";
		def_inf_provider	= "libusb-win32";
		break;
	case WDI_LIBUSBK:
		def_inf_class_guid	= "{ECFB0CFD-74C4-4f52-BBF7-343461CD72AC}";
		def_inf_class		= "libusbK devices";
		def_inf_provider	= "libusbK";
		break;
	default:
		def_inf_class_guid	= "{D9173AB3-4838-4a86-9BAD-E664C7A95927}";
		def_inf_class		= "Custom Usb devices";
		def_inf_provider	= "libusb.org";
		break;
	}

	static_strcpy(inf_entities[PWR_DEVICE_IDLE_ENABLED].replace, "1");
	static_strcpy(inf_entities[PWR_DEFAULT_IDLE_STATE].replace, "0");
	static_strcpy(inf_entities[PWR_DEFAULT_IDLE_TIMEOUT].replace, "5000");
	static_strcpy(inf_entities[PWR_USER_SET_DEVICE_IDLE_ENABLED].replace, "0");
	static_strcpy(inf_entities[PWR_DEVICE_IDLE_IGNORE_WAKE_ENABLE].replace, "0");
	static_strcpy(inf_entities[PWR_SYSTEM_WAKE_ENABLED].replace, "0");

	if (options)
	{
		if (options->pwr_device_idle_enabled != WDI_POWER_FLAG_DEFAULT)
			static_strcpy(inf_entities[PWR_DEVICE_IDLE_ENABLED].replace, (options->pwr_device_idle_enabled == WDI_POWER_FLAG_ON ? "1":"0"));
		
		if (options->pwr_default_idle_state != WDI_POWER_FLAG_DEFAULT)
			static_strcpy(inf_entities[PWR_DEFAULT_IDLE_STATE].replace, (options->pwr_default_idle_state == WDI_POWER_FLAG_ON ? "1":"0"));
		
		if (options->pwr_user_set_device_idle_enabled != WDI_POWER_FLAG_DEFAULT)
			static_strcpy(inf_entities[PWR_USER_SET_DEVICE_IDLE_ENABLED].replace, (options->pwr_user_set_device_idle_enabled == WDI_POWER_FLAG_ON ? "1":"0"));
		
		if (options->pwr_device_idle_ignore_wake_enable != WDI_POWER_FLAG_DEFAULT)
			static_strcpy(inf_entities[PWR_DEVICE_IDLE_IGNORE_WAKE_ENABLE].replace, (options->pwr_device_idle_ignore_wake_enable == WDI_POWER_FLAG_ON ? "1":"0"));
		
		if (options->pwr_system_wake_enabled != WDI_POWER_FLAG_DEFAULT)
			static_strcpy(inf_entities[PWR_SYSTEM_WAKE_ENABLED].replace, (options->pwr_system_wake_enabled == WDI_POWER_FLAG_ON ? "1":"0"));

		if (options->pwr_default_idle_timeout > 0)
		{
			static_sprintf(inf_entities[PWR_DEFAULT_IDLE_TIMEOUT].replace, "%d", options->pwr_default_idle_timeout);
		}

		if (options->inf_class_guid && options->inf_class) {
			def_inf_class_guid = options->inf_class_guid;
			def_inf_class = options->inf_class;
		}
		if (options->inf_provider)
			def_inf_provider = options->inf_provider;
	}
	static_strcpy(inf_entities[INF_CLASSGUID].replace, def_inf_class_guid);
	static_strcpy(inf_entities[INF_CLASS].replace, def_inf_class);
	static_strcpy(inf_entities[INF_PROVIDER].replace, def_inf_provider);
	...
	if ( (windows_version >= WINDOWS_VISTA) && IS_VISTA_SHELL32_AVAILABLE && (pIsUserAnAdmin()) )  {
		...
		sprintf(hw_id, "USB\\%s", options->use_wcid_driver?
