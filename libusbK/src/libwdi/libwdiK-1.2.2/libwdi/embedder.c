...
main (int argc, char *argv[])
{
	...
	CHAR creation_time_str[MAX_PATH];
	...
	for (i=0; i<nb_embeddables; i++) {
		...
		if ( (NATIVE_STAT(fullpath, &stbuf) == 0) && ((ltm = localtime(&stbuf.st_ctime)) != NULL) ) {
			sprintf(creation_time_str,"(%04d.%02d.%02d)", ltm->tm_year+1900, ltm->tm_mon+1, ltm->tm_mday);
		} else {
			creation_time_str[0]='\0';
		}
		file_time[i] = (int64_t)stbuf.st_ctime;

		printf("EMBED  %s (%s)\n", fullpath,creation_time_str);
	...
	for (last=0,i=0; i<nb_embeddables; i++) {
		...
		fprintf(header_fd, "\", \"%s\", %d, %d, INT64_C(%"PRId64"), %s },\n",
			file_name, embeddable[i].driver_type, (int)file_size[last], file_time[last], internal_name);
