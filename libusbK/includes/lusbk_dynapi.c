...
// Function typedefs:
	...
	typedef BOOL KUSB_API LibK_IsFunctionSupported_T(
	...
	typedef BOOL KUSB_API UsbK_QueryPipeEx_T(
		_in KUSB_HANDLE InterfaceHandle,
		_in UCHAR AltSettingNumber,
		_in UCHAR PipeIndex,
		_out PWINUSB_PIPE_INFORMATION_EX PipeInformationEx);

	typedef BOOL KUSB_API UsbK_GetSuperSpeedPipeCompanionDescriptor_T(
		_in KUSB_HANDLE InterfaceHandle,
		_in UCHAR AltSettingNumber,
		_in UCHAR PipeIndex,
	...
	typedef BOOL KUSB_API UsbK_IsochReadPipe_T(
	...
	typedef BOOL KUSB_API UsbK_IsochWritePipe_T(
		_in KISOCH_HANDLE IsochHandle,
		_inopt UINT DataLength,
	...
	typedef BOOL KUSB_API IsochK_Init_T(
	...
	typedef BOOL KUSB_API IsochK_Free_T(
		_in KISOCH_HANDLE IsochHandle);

	typedef BOOL KUSB_API IsochK_SetPacketOffsets_T(
		_in KISOCH_HANDLE IsochHandle,
		_in UINT PacketSize);

	typedef BOOL KUSB_API IsochK_SetPacket_T(
		_in KISOCH_HANDLE IsochHandle,
	...
	typedef BOOL KUSB_API IsochK_GetPacket_T(
		_in KISOCH_HANDLE IsochHandle,
		_in UINT PacketIndex,
	...
	typedef BOOL KUSB_API IsochK_EnumPackets_T(
		_in KISOCH_HANDLE IsochHandle,
		_in KISOCH_ENUM_PACKETS_CB* EnumPackets,
		_inopt UINT StartPacketIndex,
		_inopt PVOID UserState);

	typedef BOOL KUSB_API IsochK_CalcPacketInformation_T(
		_in BOOL IsHighSpeed,
		_in PWINUSB_PIPE_INFORMATION_EX PipeInformationEx,
		_out PKISOCH_PACKET_INFORMATION PacketInformation);

	typedef BOOL KUSB_API IsochK_GetNumberOfPackets_T(
		_in KISOCH_HANDLE IsochHandle,
		_out PUINT NumberOfPackets);

	typedef BOOL KUSB_API IsochK_SetNumberOfPackets_T(
		_in KISOCH_HANDLE IsochHandle,
		_in UINT NumberOfPackets);

	typedef BOOL KUSB_API LUsb0_ControlTransfer_T(
		_in KUSB_HANDLE InterfaceHandle,
		_in WINUSB_SETUP_PACKET SetupPacket,
		_refopt PUCHAR Buffer,
		_in UINT BufferLength,
	...
	typedef BOOL KUSB_API LUsb0_SetConfiguration_T(
		_in KUSB_HANDLE InterfaceHandle,
		_in UCHAR ConfigurationNumber);
...
static LibK_IsFunctionSupported_T* pLibK_IsFunctionSupported = NULL;
...
static UsbK_QueryPipeEx_T* pUsbK_QueryPipeEx = NULL;

static UsbK_GetSuperSpeedPipeCompanionDescriptor_T* pUsbK_GetSuperSpeedPipeCompanionDescriptor = NULL;
...
static UsbK_IsochReadPipe_T* pUsbK_IsochReadPipe = NULL;

static UsbK_IsochWritePipe_T* pUsbK_IsochWritePipe = NULL;
...
static IsochK_Init_T* pIsochK_Init = NULL;

static IsochK_Free_T* pIsochK_Free = NULL;

static IsochK_SetPacketOffsets_T* pIsochK_SetPacketOffsets = NULL;

static IsochK_SetPacket_T* pIsochK_SetPacket = NULL;

static IsochK_GetPacket_T* pIsochK_GetPacket = NULL;

static IsochK_EnumPackets_T* pIsochK_EnumPackets = NULL;

static IsochK_CalcPacketInformation_T* pIsochK_CalcPacketInformation = NULL;

static IsochK_GetNumberOfPackets_T* pIsochK_GetNumberOfPackets = NULL;

static IsochK_SetNumberOfPackets_T* pIsochK_SetNumberOfPackets = NULL;

static LUsb0_ControlTransfer_T* pLUsb0_ControlTransfer = NULL;

static LUsb0_SetConfiguration_T* pLUsb0_SetConfiguration = NULL;
...
pLibK_IsFunctionSupported = NULL;
...
pUsbK_QueryPipeEx = NULL;

pUsbK_GetSuperSpeedPipeCompanionDescriptor = NULL;
...
pUsbK_IsochReadPipe = NULL;

pUsbK_IsochWritePipe = NULL;
...
pIsochK_SetPacketOffsets = NULL;

pIsochK_SetPacket = NULL;

pIsochK_GetPacket = NULL;

pIsochK_EnumPackets = NULL;

pIsochK_CalcPacketInformation = NULL;

pIsochK_GetNumberOfPackets = NULL;

pIsochK_SetNumberOfPackets = NULL;

pLUsb0_ControlTransfer = NULL;

pLUsb0_SetConfiguration = NULL;
...
if ((pLibK_IsFunctionSupported = (LibK_IsFunctionSupported_T*)GetProcAddress(mLibusbK_ModuleHandle, "LibK_IsFunctionSupported")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function LibK_IsFunctionSupported.\n");
}
...
if ((pUsbK_QueryPipeEx = (UsbK_QueryPipeEx_T*)GetProcAddress(mLibusbK_ModuleHandle, "UsbK_QueryPipeEx")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function UsbK_QueryPipeEx.\n");
}

if ((pUsbK_GetSuperSpeedPipeCompanionDescriptor = (UsbK_GetSuperSpeedPipeCompanionDescriptor_T*)GetProcAddress(mLibusbK_ModuleHandle, "UsbK_GetSuperSpeedPipeCompanionDescriptor")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function UsbK_GetSuperSpeedPipeCompanionDescriptor.\n");
}
...
if ((pUsbK_IsochReadPipe = (UsbK_IsochReadPipe_T*)GetProcAddress(mLibusbK_ModuleHandle, "UsbK_IsochReadPipe")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function UsbK_IsochReadPipe.\n");
}

if ((pUsbK_IsochWritePipe = (UsbK_IsochWritePipe_T*)GetProcAddress(mLibusbK_ModuleHandle, "UsbK_IsochWritePipe")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function UsbK_IsochWritePipe.\n");
}
...
if ((pIsochK_Init = (IsochK_Init_T*)GetProcAddress(mLibusbK_ModuleHandle, "IsochK_Init")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function IsochK_Init.\n");
}

if ((pIsochK_Free = (IsochK_Free_T*)GetProcAddress(mLibusbK_ModuleHandle, "IsochK_Free")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function IsochK_Free.\n");
}

if ((pIsochK_SetPacketOffsets = (IsochK_SetPacketOffsets_T*)GetProcAddress(mLibusbK_ModuleHandle, "IsochK_SetPacketOffsets")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function IsochK_SetPacketOffsets.\n");
}

if ((pIsochK_SetPacket = (IsochK_SetPacket_T*)GetProcAddress(mLibusbK_ModuleHandle, "IsochK_SetPacket")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function IsochK_SetPacket.\n");
}

if ((pIsochK_GetPacket = (IsochK_GetPacket_T*)GetProcAddress(mLibusbK_ModuleHandle, "IsochK_GetPacket")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function IsochK_GetPacket.\n");
}

if ((pIsochK_EnumPackets = (IsochK_EnumPackets_T*)GetProcAddress(mLibusbK_ModuleHandle, "IsochK_EnumPackets")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function IsochK_EnumPackets.\n");
}

if ((pIsochK_CalcPacketInformation = (IsochK_CalcPacketInformation_T*)GetProcAddress(mLibusbK_ModuleHandle, "IsochK_CalcPacketInformation")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function IsochK_CalcPacketInformation.\n");
}

if ((pIsochK_GetNumberOfPackets = (IsochK_GetNumberOfPackets_T*)GetProcAddress(mLibusbK_ModuleHandle, "IsochK_GetNumberOfPackets")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function IsochK_GetNumberOfPackets.\n");
}

if ((pIsochK_SetNumberOfPackets = (IsochK_SetNumberOfPackets_T*)GetProcAddress(mLibusbK_ModuleHandle, "IsochK_SetNumberOfPackets")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function IsochK_SetNumberOfPackets.\n");
}

if ((pLUsb0_ControlTransfer = (LUsb0_ControlTransfer_T*)GetProcAddress(mLibusbK_ModuleHandle, "LUsb0_ControlTransfer")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function LUsb0_ControlTransfer.\n");
}

if ((pLUsb0_SetConfiguration = (LUsb0_SetConfiguration_T*)GetProcAddress(mLibusbK_ModuleHandle, "LUsb0_SetConfiguration")) == NULL)
{
	funcLoadFailCount++;
	OutputDebugStringA("Failed loading function LUsb0_SetConfiguration.\n");
}
	    ...
	    _in INT DriverID)
{
	...
	KUSB_EXP BOOL KUSB_API LibK_IsFunctionSupported(
		...
		_in UINT FunctionID)
{
	return pLibK_IsFunctionSupported(DriverAPI, FunctionID);
}
	    ...
	    _out PWINUSB_PIPE_INFORMATION PipeInformation)
{
	...
	KUSB_EXP BOOL KUSB_API UsbK_QueryPipeEx(
		...
		_out PWINUSB_PIPE_INFORMATION_EX PipeInformationEx)
{
	return pUsbK_QueryPipeEx(InterfaceHandle, AltSettingNumber, PipeIndex, PipeInformationEx);
}
	...
	KUSB_EXP BOOL KUSB_API UsbK_GetSuperSpeedPipeCompanionDescriptor(
		...
		_out PUSB_SUPERSPEED_ENDPOINT_COMPANION_DESCRIPTOR PipeCompanionDescriptor)
{
	return pUsbK_GetSuperSpeedPipeCompanionDescriptor(InterfaceHandle, AltSettingNumber, PipeIndex, PipeCompanionDescriptor);
}
	    ...
	    _out PUINT FrameNumber)
{
	...
	KUSB_EXP BOOL KUSB_API UsbK_IsochReadPipe(
		...
		_in LPOVERLAPPED Overlapped)
{
	return pUsbK_IsochReadPipe(IsochHandle, DataLength, FrameNumber, NumberOfPackets, Overlapped);
}
	...
	KUSB_EXP BOOL KUSB_API UsbK_IsochWritePipe(
		...
		_in LPOVERLAPPED Overlapped)
{
	return pUsbK_IsochWritePipe(IsochHandle, DataLength, FrameNumber, NumberOfPackets, Overlapped);
}
	    ...
	    _ref PKISO_CONTEXT IsoContext)
{
	...
	KUSB_EXP BOOL KUSB_API IsochK_Init(
		...
		_in UINT TransferBufferSize)
{
	return pIsochK_Init(IsochHandle, InterfaceHandle, PipeId, MaxNumberOfPackets, TransferBuffer, TransferBufferSize);
}

	KUSB_EXP BOOL KUSB_API IsochK_Free(
		_in KISOCH_HANDLE IsochHandle)
{
	return pIsochK_Free(IsochHandle);
}
	...
	KUSB_EXP BOOL KUSB_API IsochK_SetPacketOffsets(
		...
		_in UINT PacketSize)
{
	return pIsochK_SetPacketOffsets(IsochHandle, PacketSize);
}
	...
	KUSB_EXP BOOL KUSB_API IsochK_SetPacket(
		...
		_in UINT Status)
{
	return pIsochK_SetPacket(IsochHandle, PacketIndex, Offset, Length, Status);
}
	...
	KUSB_EXP BOOL KUSB_API IsochK_GetPacket(
		...
		_outopt PUINT Status)
{
	return pIsochK_GetPacket(IsochHandle, PacketIndex, Offset, Length, Status);
}
	...
	KUSB_EXP BOOL KUSB_API IsochK_EnumPackets(
		...
		_inopt PVOID UserState)
{
	return pIsochK_EnumPackets(IsochHandle, EnumPackets, StartPacketIndex, UserState);
}
	...
	KUSB_EXP BOOL KUSB_API IsochK_CalcPacketInformation(
		...
		_out PKISOCH_PACKET_INFORMATION PacketInformation)
{
	return pIsochK_CalcPacketInformation(IsHighSpeed, PipeInformationEx, PacketInformation);
}
	...
	KUSB_EXP BOOL KUSB_API IsochK_GetNumberOfPackets(
		...
		_out PUINT NumberOfPackets)
{
	return pIsochK_GetNumberOfPackets(IsochHandle, NumberOfPackets);
}
	...
	KUSB_EXP BOOL KUSB_API IsochK_SetNumberOfPackets(
		...
		_in UINT NumberOfPackets)
{
	return pIsochK_SetNumberOfPackets(IsochHandle, NumberOfPackets);
}
	...
	KUSB_EXP BOOL KUSB_API LUsb0_ControlTransfer(
		...
		_inopt LPOVERLAPPED Overlapped)
{
	return pLUsb0_ControlTransfer(InterfaceHandle, SetupPacket, Buffer, BufferLength, LengthTransferred, Overlapped);
}
	...
	KUSB_EXP BOOL KUSB_API LUsb0_SetConfiguration(
		...
		_in UCHAR ConfigurationNumber)
{
	return pLUsb0_SetConfiguration(InterfaceHandle, ConfigurationNumber);
}
