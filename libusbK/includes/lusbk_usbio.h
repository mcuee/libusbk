...
/*!
*
* The \ref KUSB_ISO_CONTEXT::StartFrame member of the \ref
* KUSB_ISO_CONTEXT specifies the starting USB frame number for the
...
* instance, the fourth packet in the \ref KUSB_ISO_CONTEXT has an index of
...
*
* When the driver processes the \ref KUSB_ISO_CONTEXT, it discards all
* packets in the \ref KUSB_ISO_CONTEXT whose frame numbers are lower than
...
* KUSB_ISO_CONTEXT whose frame numbers are higher than the current frame
* number.
*
...
* example, if the StartFrame value recorded in the \ref KUSB_ISO_CONTEXT
...
* will discard when a \ref KUSB_ISO_CONTEXT arrives late. Assume the
...
* isochronous data for the pipe. Therefore, if the \ref KUSB_ISO_CONTEXT
...
* frame and the StartFrame value specified in the \ref KUSB_ISO_CONTEXT
...
* KUSB_ISO_PACKET \c USBD_STATUS_BAD_START_FRAME and discards the entire
* \ref KUSB_ISO_CONTEXT. The following code example shows the precise
* check that the port driver does on the \ref KUSB_ISO_CONTEXT start
* frame:
* \code
* if (abs((CurrentFrame - StartFrame)) > USBD_ISO_START_FRAME_RANGE)
* {
* 	// discard the KUSB_ISO_CONTEXT
* }
* \endcode
*
*/
typedef struct _KUSB_ISO_CONTEXT
{
	...
	* KUSB_ISO_CONTEXT::StartFrame, and the bus driver will begin the
	* transaction in the next available frame.
	*
	* Specifing \b 0 for \ref KUSB_ISO_CONTEXT::StartFrame (start transfer
...
typedef BOOL KUSB_API ISO_ENUM_PACKETS_CB (__in ULONG PacketIndex, __inout PKUSB_ISO_PACKET IsoPacket, __inout PVOID UserContext);
