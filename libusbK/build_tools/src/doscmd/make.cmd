...
REM :: START OF MAIN SECTION -------------------------------------------
	...
	CALL :AddIfDef WDK_ENV_OPTIONS G_FRE G_CHK G_64 G_X64 G_WIN7 G_WLH G_WXP G_WNET G_W2K G_NO_OACR
...
:FormatCode
	...
	"!G_FORMATCODE_EXE!" !G_FORMATCODE_ARGS!
GOTO :EOF
...
GOTO :EOF
	...
	:Clean_Bin
	:Clean_Output
	CALL :SyncPathList D __Clean_AllDirList  "!G_BUILD_OUTPUT_BASE_DIR!" "/AD /S"  G_CLEAN_BIN_EXP "/I" @NULL ""
	CALL :SyncPathList F __Clean_AllFileList "!G_BUILD_OUTPUT_BASE_DIR!" "/A-D /S" G_CLEAN_BIN_EXP "/I" @NULL ""
	GOTO :EOF
	...
	CALL :SyncPathList D __Clean_AllDirList  "!CD!" "/AD /S"  G_CLEAN_SRC_EXP "/I" __Clean_RegExpExcludeList "/I /V"
	CALL :SyncPathList F __Clean_AllFileList "!CD!" "/A-D /S" G_CLEAN_SRC_EXP "/I" __Clean_RegExpExcludeList "/I /V"
	GOTO :EOF
GOTO :EOF
...
GOTO :EOF
	...
	CALL :SyncPathList F __Zip_FileList "!G_BUILD_OUTPUT_BASE_DIR!" "/A-D /S" G_CLEAN_BIN_EXP "/I /V" @NULL ""
	...
	CALL :SyncPathList F __Zip_FileList "!CD!" "/A-D /S" G_CLEAN_SRC_EXP "/I /V" __Zip_RegExpExcludeList "/I /V"
	IF "%~1" EQU "" (
	...
	FOR /F "usebackq eol=; tokens=* delims=" %%I IN (`!DCMD! "-sp=;" "!G_WDK_SOURCES_LIST!"`) DO (
		...
		IF "!__SOURCES_FILENAME!" NEQ "" (
			...
			IF EXIST "!__BUILD_VERSION_FILE!" (
				CALL :Print D "BUILD" "VERSION_FILE=@s TARGET_NAME=@s" "!__BUILD_VERSION_FILE!" "!__SOURCES_NAME!"
				SET VERSION_FILENAME="!__SOURCES_NAME!"
				!DCMD! -vc=tag "!__BUILD_VERSION_FILE!" "!__SOURCES_NAME!"
			)
...
:Build_Sign
	IF DEFINED G_SIGN (
		...
		SET __SIGN_EXTS=dll sys exe
		CALL :PtrToString __SIGN_EXTS_CMP __SIGN_EXTS:%~x1=
		IF "!__SIGN_EXTS!" EQU "!__SIGN_EXTS_CMP!" (
...
:Build_PostTasks
	...
	CALL :SyncPathList F __Build_FileList "!G_BUILD_OUTPUT_BASE_DIR!" "/AA-D /S" G_CLEAN_BIN_EXP "/I /V" @NULL ""
...
:KitSetup
	IF DEFINED DDKBUILDENV (
		IF EXIST "!PUBLIC_ROOT!\bin\setenv.bat" (
			IF /I "!G_WDK_DIR!" NEQ "!PUBLIC_ROOT!" (
...
:SafeCall
	...
	IF NOT "!ERRORLEVEL!" EQU "0" (
		CALL :SetError 1
		CALL :Print E "ERROR" "Failed executing command: @s" "!SafeCall_Command!"
	)
GOTO :EOF
